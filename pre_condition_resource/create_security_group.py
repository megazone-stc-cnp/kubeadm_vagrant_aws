...
def main(NAME):
    ...
    get_subnets(vpc_id)
    is_find = find_security_group(NAME, vpc_id)
    any_cidr = "0.0.0.0/0"

    if is_find is False:
        create_security_group(NAME, vpc_id)
        add_ingress_rule(NAME, f"{my_ip}/32", 22, "ssh connect", "tcp")
        add_ingress_rule(NAME, f"{default_cidr}", 22, "ssh connect", "tcp")
        add_ingress_rule(NAME, f"{default_cidr}", 6443, "Master Node kube-apiserver", "tcp")
        add_ingress_rule(NAME, f"{default_cidr}", 10250, "Master Node kubelet", "tcp")
        add_ingress_rule(NAME, f"{default_cidr}", 2379, "Master Node kubelet", "tcp")
        add_ingress_rule(NAME, f"{default_cidr}", 2380, "Master Node kubelet", "tcp")
        add_ingress_rule(NAME, f"{any_cidr}", 51820, "calico", "udp")
        add_ingress_rule(NAME, f"{any_cidr}", 179, "calico", "tcp")
        add_ingress_rule(NAME, f"{default_cidr}", 443, "Master Node kube-apiserver", "tcp")
        add_ingress_rule(NAME, f"{default_cidr}", 5473, "calico", "tcp")
        add_ingress_rule(NAME, f"{any_cidr}", 4789, "calico", "udp")
        add_ingress_rule(NAME, f"{any_cidr}", 51821, "calico", "udp")
